...
// Local hidden-product filter. Uses relative path to reach our overridden constants
// (importing via '@salesforce/retail-react-app/app/constants' from inside an override
// resolves to the ORIGINAL constants file due to the pwa-kit self-loop guard).
...
const ProductList = (props) => {
    ...
    // Strip hidden products (e.g. iPods) from the hits array so they don't
    // appear in category listings or search results. Also adjust the total
    // count so the "N results" label stays accurate. If every hit gets
    // filtered out, delete the hits key so the base template's "no results"
    // branch (which checks `!productSearchResult?.hits`) fires.
    if (productSearchResult?.hits?.length) {
        const before = productSearchResult.hits.length
        productSearchResult.hits = productSearchResult.hits.filter(
            (h) => !isHiddenProductId(h.productId)
        )
        const removed = before - productSearchResult.hits.length
        if (removed > 0 && typeof productSearchResult.total === 'number') {
            productSearchResult.total = Math.max(0, productSearchResult.total - removed)
        }
        if (productSearchResult.hits.length === 0) {
            delete productSearchResult.hits
        }
    }
